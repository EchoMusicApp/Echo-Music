...
val MIGRATION_PLAYLIST_SAFEGUARD = object : Migration(40, 42) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("DROP TABLE IF EXISTS `brain_activity_log`")
        db.execSQL("DROP TABLE IF EXISTS `play_event`")
        ...
        db.query("PRAGMA table_info(playlist)").use { cursor ->
            ...
            if (!columns.contains("isLocal")) {
                db.execSQL("ALTER TABLE playlist ADD COLUMN isLocal INTEGER NOT NULL DEFAULT 0")
            }
            if (!columns.contains("isAutoSync")) {
                db.execSQL("ALTER TABLE playlist ADD COLUMN isAutoSync INTEGER NOT NULL DEFAULT 0")
            }
            if (!columns.contains("isEditable")) {
                db.execSQL("ALTER TABLE playlist ADD COLUMN isEditable INTEGER NOT NULL DEFAULT 1")
            }
        }
    }
}
